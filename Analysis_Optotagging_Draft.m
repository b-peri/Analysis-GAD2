...
% Inhibited units, 3. (Putative) disinhibited units!
...
for intCl = 1:intNumClu
    if ismember(sAP.sCluster(intCl).Area, vecROI) %selection criteria: Area, quality criteria?
        ...
        [dblZetaP,~,sRate,vecLatency] = zetatest(vecSpikeTimes,vecLaserOnSecs(pulseDurTrial == 0.02)-0.5,1,[],4,3);
        ...
        if ~(PeakLat_Cl < 0.501) && (PeakLat_Cl < 0.51);
            CellResp = "GAD2+";
        else
